...
func main() {
	if len(os.Args) != 2{
		fmt.Fprintf(os.Stderr, "Usage: %s hpst", os.Args[0])
		os.Exit(1)
	}
	dst := os.Args[1]
	raddr, err := net.ResolveIPAddr("ip4", dst)
	checkError(err)
	ipconn, err := net.DialIP("ip:icmp", nil, raddr)
	checkError(err)
	sendid := os.Getpid() & 0xfffff
	sendseq := 1
	pingpktlen := 64
	for {
		...
		for {
			...
			checkError(err)
			fmt.Printf("%d bytes from %s: icmp_req = %d time = %2.f ms\n",n, from, sendseq, elapsedTime(start))
